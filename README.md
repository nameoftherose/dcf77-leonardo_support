...
- If possible use a a crystal with the highest frequency stability and adjust `static const int16_t max_total_adjust = 6400;` accordingly. In any case adjust it to the stability of the crystal used. The default value (corresponding to 400ppm) is way too high, destabilizing the algorithm and decreasing the chances for reaquiring lock after a prolong period of fading.
