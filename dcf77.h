...
extern uint8_t year_error;
 ...
 namespace Internal {
     ...
     struct DCF77_Local_Clock {
         ...
         void setup() {
             sprint("DCF77_Clock::setup (.h L1320) Intitializing the Local_Clock to state ");
             clock_state = Clock::free/*useless*/;
             sprintln(clock_state);
             tick = 0;
             unlocked_seconds = 0;
         ...
         void process_1_Hz_tick(const DCF77_Encoder &decoded_time) {/*decoded_time is NOT the time estimate */
             year_val=Clock_Controller::Year_Decoder.get_time_value().val;
             if (year_val <0x17){
                 year_error_count++;
                 Clock_Controller::Year_Decoder.setup();
                 Clock_Controller::Month_Decoder.setup();
                 Clock_Controller::Day_Decoder.setup();
                 Clock_Controller::Weekday_Decoder.setup();
             }
             if (year_error){
                 switch (year_error){
                     case 2:Clock_Controller::Hour_Decoder.setup();
                     case 1:{
                        Clock_Controller::Year_Decoder.setup();
                        Clock_Controller::Month_Decoder.setup();
                        Clock_Controller::Day_Decoder.setup();
                 ...
                 switch (clock_state) {
                     ...
                     case Clock::locked: {
                         if (  Clock_Controller::get_demodulator_quality_factor() > unacceptable_demodulator_quality
                            && Clock_Controller::Second_Decoder.signal_max-Clock_Controller::Second_Decoder.noise_max >= Clock_Controller::Second_Decoder.lock_threshold) {
     ...
     struct DCF77_Clock_Controller {
         ...
         static uint8_t get_overall_quality_factor() {
             using namespace Arithmetic_Tools;
             //lock_quality_t seconds_quality;
             //Second_Decoder.get_quality(seconds_quality);
             //if (seconds_quality.lock_max-seconds_quality.noise_max<99) return 0;
             if (Second_Decoder.signal_max-Second_Decoder.noise_max<Second_Decoder.lock_threshold) return 0;
             if (Year_Decoder.get_time_value().val<0x17) return 0;
         ...
         static uint8_t get_demodulator_quality_factor() {
             return /*(Year_Decoder.get_time_value().val<0x17)?0:*/Demodulator.get_quality_factor();
         }
         ...
         static void setup() {
             sprintln("Clock_Controller::setup .h L2143");
             Local_Clock.setup();
             Demodulator.setup();
